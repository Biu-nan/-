...
export class DianxiaomiAdapter {
  ...
  // 无线端描述：改用「使用新版编辑器 → 在弹窗中点击根据PC端描述一键生成 → 保存」。
  // 关键坑（已实测）：
  //   1) 点击「使用新版编辑器」后会打开一个标题为「新版编辑器」的 Ant Modal。
  //   2) 在弹窗顶部点击「根据PC端描述一键生成」，即可从 PC 描述同步。
  //   3) 同步完成后点击「保存」关闭弹窗，无线端描述即生效。
  //   4) PC 端描述必须先于本方法完成（含插图），否则生成内容缺失。
  async clickNewEditorThenGenerateFromPc() {
    await this.connect();
    const modalSelector = ".ant-modal-wrap, .ant-modal-mask";
    const hasModal = async () => this.page.locator(".ant-modal-content").filter({ hasText: /新版编辑器|根据PC端描述一键生成/ }).count().then((n) => n > 0);
    const hasGenerateBtn = async () => this.page.locator(".ant-modal-content").filter({ hasText: /新版编辑器/ }).getByText("根据PC端描述一键生成", { exact: true }).count().then((n) => n > 0);

    // 若弹窗已存在且已有生成按钮，直接点生成
    if (await hasModal() && await hasGenerateBtn()) {
      const genBtn = this.page.locator(".ant-modal-content").filter({ hasText: /新版编辑器/ }).getByText("根据PC端描述一键生成", { exact: true }).first();
      await genBtn.click({ timeout: 8000 }).catch(() => {});
      await sleep(6000);
      const save = this.page.locator(".ant-modal-content").filter({ hasText: /新版编辑器/ }).getByText("保存", { exact: true }).first();
      if (await save.count()) { await save.click({ timeout: 5000 }).catch(() => {}); await sleep(1500); }
      return { ok: true, autoGenerated: true };
    }
    ...
    // 2) 等弹窗出来
    let waited = 0;
    while (!(await hasModal()) && waited < 8000) { await sleep(500); waited += 500; }
    if (!(await hasModal())) return { ok: false, reason: "modal-not-opened" };

    // 3) 在弹窗里点「根据PC端描述一键生成」
    let genBtn = this.page.locator(".ant-modal-content").filter({ hasText: /新版编辑器/ }).getByText("根据PC端描述一键生成", { exact: true }).first();
    if (await genBtn.count() === 0) {
      // 有些版本按钮文字在 title/toolbar 上，尝试模糊匹配
      genBtn = this.page.locator(".ant-modal-content").filter({ hasText: /新版编辑器/ }).getByText(/PC端描述.*一键生成|一键生成/).first();
    }
    if (await genBtn.count() === 0) return { ok: false, reason: "no-generate-btn-in-modal" };
    await genBtn.click({ timeout: 8000 }).catch(() => {});
    await sleep(6000);
    ...
    if (await save.count()) { await save.click({ timeout: 5000 }).catch(() => {}); await sleep(1500); }
      ...
      const close = this.page.locator(".ant-modal-content").filter({ hasText: /新版编辑器/ }).getByText("关闭", { exact: true }).first();
      if (await close.count()) { await close.click({ timeout: 5000 }).catch(() => {}); await sleep(1500); }
    }
    return { ok: true };
  }
